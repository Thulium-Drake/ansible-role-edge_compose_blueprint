...
You can also override the current list of files in the module at buildtime by overwriting the contents of `%{builddir}/files.list`. This file is generated by Ansible before the rpmbuild task is run. For example:

```
%build
# Do magic that will change files in the buildroot
find %{buildroot} -type f | sed 's|%{buildroot}||' > %{_builddir}/files.list
```
